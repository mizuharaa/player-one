...
export function registerSettle(
   ...
   * payout lane uses.
   *
   * Parking a settlement in exception and releasing it move the same money a
   * read only shows, and until F5's fix they carried no role check at all —
   * measured the same way: a plain counter operator at any centre, or an
   * administrator, could do both. They carry this guard too now.
   *
   * The one write that stays on plain `requireActor` is `/api/settle/bills`:
   * 0013 needs the cycle generated by someone other than finance, so that
   * route checks the role itself and refuses finance — see
   * `settle_generate_by_finance` below.
   */
   ...
   */
  app.post('/api/settle/settlements/:id/exception', readOpts, async (req, reply) => {
    const id = pathId(req);
   ...
   */
  app.post('/api/settle/settlements/:id/release', readOpts, async (req, reply) => {
    const id = pathId(req);
    if (id === null) return reply.code(400).send({ error: 'invalid id' });
