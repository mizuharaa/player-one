...
/**
 * The READ half of the same gate: finance, or the administrator.
 *
 * Reading money and moving money are two different questions, and the guard
 * above answered both with one word. That is why the demo's single
 * administrator credential could not see a bill it is expected to debug.
 *
 * Widened for reads only, and only to `administrator` — never to a counter
 * operator, which is the hole `financeGuard` was lifted here to close. Every
 * POST on the payout, settle and risk lanes keeps `financeGuard`, and so do the
 * two export routes, so an administrator can look at everything finance looks
 * at and can still change nothing. The separation-of-duty rules are untouched:
 * they live in the schema (`payout_finance_in_transaction`, 0018 and 0031) and
 * in `settle.ts`'s refusal of a bill generated by finance, and this guard is
 * not on any of those paths.
 */
export const financeReadGuard =
  (db: Db) =>
  async (req: FastifyRequest, reply: Reply): Promise<unknown> => {
    const role = await roleOf(db, req.actor);
    if (role !== 'finance' && role !== ADMIN_ROLE) {
      return reply.code(403).send({ error: 'finance role required' });
    }
    return undefined;
  };
