...
describe.skipIf(!hasDb())('parking an episode out of the queue', () => {
  ...
  /**
   * The surface. Two branches each built an exit for a stuck episode and
   * neither built a way to find one, so an episode that left the queue left
   * every screen with it: no list, no id, and `?queue=held` is a 400. The loop
   * was replaced by a silent hole — the collector is still not paid, and now
   * nobody trips over it.
   *
   * This is the one route an operator reads. Both mechanisms appear in it,
   * because an episode can be held and parked at once and lifting one is not
   * lifting the other.
   */
  it('lists every episode that is out of the queue, held or parked, and names the route back', async () => {
    const h = await harness();
    const nothing = await h.send('GET', '/episodes/stuck');
    expect(nothing.statusCode, nothing.body).toBe(200);
    expect(nothing.json().episodes).toEqual([]);

    // One episode parked by an operator; nobody ever claimed it.
    const { episodeId: parked } = await h.deliver();
    expect((await h.park(parked, { reason: 'the card must come back' })).statusCode).toBe(200);

    // A second episode a reviewer claimed and then held at verdict time.
    const { episodeId: held } = await h.deliver(h.batch, h.headers, 'AZER76400FE', 120);
    // Claimed and held through the counter session, like every other claim in
    // this file: a PaXini reviewer session is refused 451 on claim while D11
    // withholds remote playback.
    const claim = await h.send('POST', '/api/review/claim', { queue: 'standard' });
    expect(claim.statusCode, claim.body).toBe(200);
    const hold = await h.send('POST', `/api/review/hold/${claim.json().episode_id}`, { reason: 'no task on this session' });
    expect(hold.statusCode, hold.body).toBe(200);

    const res = await h.send('GET', '/episodes/stuck');
    expect(res.statusCode, res.body).toBe(200);
    const byId = new Map<string, Record<string, unknown>>(
      (res.json().episodes as { episode_id: string }[]).map((e) => [e.episode_id, e as never]),
    );
    expect(byId.size).toBe(2);

    const p = byId.get(parked) as { park: Record<string, unknown>; held: unknown };
    expect(p.held).toBeNull();
    expect(p.park["reason"]).toBe('the card must come back');
    expect(p.park.parked_by).toBe('op');
    expect(p.park.release_with).toBe(`POST /episodes/${parked}/unpark`);

    const q = byId.get(claim.json().episode_id) as { park: unknown; held: Record<string, unknown> };
    expect(q.park).toBeNull();
    expect(q.held.release_with).toBe(`POST /api/review/hold/${claim.json().episode_id}`);

    // The other centre's operator sees neither: SEC-02, same scope as the rest
    // of this file.
    const other = await h.send('GET', '/episodes/stuck', undefined, h.headers2);
    expect(other.statusCode, other.body).toBe(200);
    expect(other.json().episodes).toEqual([]);

    // Releasing the park does not release the hold, and the list says so
    // rather than dropping the episode.
    await h.unpark(parked, { reason: 'card came back' });
    const after = await h.send('GET', '/episodes/stuck');
    expect((after.json().episodes as { episode_id: string }[]).map((e) => e.episode_id)).toEqual([
      claim.json().episode_id,
    ]);
  });
});
