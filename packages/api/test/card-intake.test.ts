...
/**
 * `scripts/card-intake.mjs` — the counter's one command — against a real
 * sample session, the real routes and a real Postgres.
 *
 * The retry rule is what this file exists for. P0-2's acceptance says "import
 * and upload retried once with no duplicate episode or bill", and the shape
 * that hides a broken retry is a fixture where the second run cannot collide:
 * `counter.ts import` mints a fresh handover, batch and session id every time,
 * so a retry always looked fine and always left a second batch and a second
 * declared session behind. Here the same card path is intaken three times —
 * before the reviewer sees it, after the verdict, and after finance has a bill
 * — and every row the retry could double is counted at each step.
 *
 ...
 * is the path an operator now takes. The last run is given the copy instead,
 * which is the other branch. Nothing in here writes to the corpus, and the
 * last test measures that.
 *
...
describe.skipIf(!hasDb() || !hasSession(SESSION))('one-command card intake', () => {
  const ids = {
    centre: uid(), machine: uid(), operator: uid(), finance: uid(),
    collector: uid(), deviceType: uid(), device: uid(), task: uid(), scenario: uid(),
  };
  const episodeId = deriveEpisodeId(basename(session(SESSION)));
  ...
  function intake(sessionDir: string): Promise<Run> {
  ...
  beforeAll(async () => {
    ...
    onCard = await sizes(session(SESSION));
  ...
  it('copies the card, matches every checksum, and lands one verified episode', async () => {
    const run = await intake(session(SESSION));
    expect(run.code, run.stderr).toBe(0);
    expect(run.table).toMatchObject({
      session: basename(session(SESSION)),
      episode: episodeId,
      ingest: 'new',
    ...
    expect(run.table.batch).toMatch(/\(opened, handover opened\)$/);
  ...
  it('prints duplicate on a second run and counts nothing twice', async () => {
    ...
    // The same batch and the same handover, reused rather than opened again.
    expect(run.table.batch).toMatch(/\(reused, handover reused\)$/);
  ...
  it('turns one verdict into one bill line whose minutes reproduce its amount', async () => {
    ...
    expect(claimed.json().episode_id).toBe(episodeId);
  ...
  it('leaves the bill alone when the copy is intaken a third time, and never wrote to the card', async () => {
    const before = await counts();
    // The copy's own path this time: already inside the media root, so nothing
    // is copied again.
    const run = await intake(join(inbox, basename(session(SESSION))));
    expect(run.code, run.stderr).toBe(0);
    expect(run.table).toMatchObject({
    ...
    expect(await sizes(session(SESSION))).toEqual(onCard);
  }, 600_000);
});
