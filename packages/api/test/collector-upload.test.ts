...
describe.skipIf(!hasDb())('Path A, the collector upload', () => {
  ...
  async function harness(store: MemoryStore = new MemoryStore()) {
     ...
     */
    const tokenFor = (collectorId: string): Record<string, string> => ({
  ...
  it('answers 503 when no object store is configured', async () => {
    ...
    const res = await app.inject({
      ...
      headers: { authorization: `Bearer ${signToken(SECRET, { kind: 'collector', collectorId: uid() })}` },
    });
...
describe.skipIf(!hasDb() || !hasStore())('Path A against a real S3 endpoint', () => {
  ...
  async function harness() {
    ...
    await app.ready();
    const headers = {
      authorization: `Bearer ${signToken(SECRET, { kind: 'collector', collectorId: ids.collector })}`,
    };
