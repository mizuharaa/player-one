...
| `PLAYERONE_DEBUG_DELIVERY` | | `1` shows the console's **Debug delivery** page under Engineering, for administrators only. The page signs in as a collector with `PLAYERONE_DEMO_PHONE`, hashes a session directory the operator picks in the browser, and pushes it through the phone's own routes. It adds no route and grants no access — it is a face on `POST /api/me/uploads` — but it is debug tooling and `deploy/centre/check.mjs` refuses it in production. A direct browser PUT also needs a CORS rule on the bucket: `node packages/api/scripts/bucket-cors.mjs <console origin>`. |
