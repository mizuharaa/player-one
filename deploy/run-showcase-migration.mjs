...
const NAME = 'playerone-showcase-migration-0026-0027';
...
const allowed = ['.dockerignore', 'Dockerfile', 'railway.toml', 'manifest.json', 'apply.sql', 'run.sh', '0026_showcase_footage.sql', '0027_showcase_review_grants.sql'];
...
const expected = [['0026_showcase_footage', 1788990000000], ['0027_showcase_review_grants', 1788990010000]];
if (manifest.migrations?.length !== 2 || manifest.secretsIncluded !== false) throw new Error('Unexpected migration bundle');
...
const statePath = join(root, 'scratchpad/qa/showcase-migration-job-state.json');
...
while (Date.now() < deadline) {
  ...
  if (latest) {
    ...
    if (['SUCCESS', 'COMPLETED', 'REMOVED', 'FAILED', 'CRASHED'].includes(latest.status)) {
      ...
      await writeFile(join(root, 'scratchpad/qa/showcase-migration-job.log'), sanitize(logs));
...
console.log(JSON.stringify({ migration: '0026+0027', exit: 0, hash, serviceRemoved: true, evidence: statePath }));
