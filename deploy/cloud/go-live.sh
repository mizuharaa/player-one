...
usage() { echo "Usage: bash deploy/cloud/go-live.sh <ip> [--domain D] [--bucket B] [--acme-email E] [--ssh-port N] [--ssh-user U] [--ssh-key PATH] [--quota-bytes N] [--zalo-app-id ID --zalo-app-secret S] [--sign-in-channel C] [--demo-bypass-key K] [--force] [--dry-run|--plan]" >&2; exit 2; }
[[ $# -ge 1 ]] || usage
ip=$1; shift
