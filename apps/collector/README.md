...
session, confirm an upload, watch the income line. Expo SDK 57 / React Native 0.86,
TypeScript strict, Vietnamese first (LOC-01 is P0; English rides along at P2).

## What this is, exactly

**A real client against the platform, one mocked seam left, packaged for
Expo Go and a self-contained Android APK.** Phone checks remain unverified.
...
| Server | `HttpCollectorApi` over `fetch`, against the fourteen `/api/me/*` routes plus `/api/me/income` and `/api/me/episodes`. `MockCollectorApi` is still selectable with `EXPO_PUBLIC_MOCK_API=1` and is what the screen tests run on. |
...
| Native project | Generated by `expo prebuild`; `android/` and `ios/` are ignored, never committed. Expo supplies the default Metro and Babel configuration. Secure storage remains unexercised on real hardware. |
| Launchable | `pnpm -F @playerone/collector start` serves Expo Go; `android` runs a local Android development build; `apk` builds a release APK with an embedded bundle. Built and inspected on 2026-09-08: 81,495,621 bytes, package `vn.vng.playerone.collector`, minSdk 28, ABIs arm64-v8a, armeabi-v7a, x86 and x86_64. Opening it on a phone remains unverified; there is no phone here. |

The typechecker and unit tests cover the
...
Set `EXPO_PUBLIC_API_URL=http://<this PC's LAN address>:8080` in
`apps/collector/.env` beside `package.json`, or export it in the shell:

```sh
export EXPO_PUBLIC_API_URL=http://192.168.1.10:8080
pnpm -F @playerone/collector start
```

Replace the example address with the API PC's LAN address, and start the API
bound to it: `HOST=<that address> pnpm serve`. The API listens on `127.0.0.1`
by default, which a phone cannot reach. Put the phone on the same network and
scan the QR code the CLI prints with Expo Go. Leave
`EXPO_PUBLIC_MOCK_API` unset to use the real server; set it to `1` for the mock.

## Build the Android APK

From the repository root in Git Bash, with JDK 17 and the Android SDK installed
and SDK licences accepted:
...
Two things about this build are Windows, not Expo, and both are already set in
the repository's `.npmrc`. `node-linker=hoisted` is required: with pnpm's
default layout `expo-modules-core` reaches `react-native` through a symlink,
CMake keeps deciding its own build file is out of date, and the build dies with
`ninja: error: manifest 'build.ninja' still dirty after 100 tries`. The `apk`
script also spells the launcher `.\gradlew`, because this machine sets
`NoDefaultCurrentDirectoryInExePath`, so `cmd.exe` will not run a program from
the current directory by bare name.
The release variant includes the JavaScript bundle and runs without Metro.
It uses the template's debug keystore; a real release keystore is an operations
decision that has not been taken.

<!-- Cleartext HTTP is enabled only for this LAN smoke-test artifact. -->

Phone checks remain the owner's: use the iPhone with Expo Go and an Android
9+ phone with a supported ABI for the APK. On the sign-in screen, enter a phone
number and tap send code; verify `POST /auth/collector/request-code` appears
in the API server log. Install and open the APK with Metro stopped to prove
the bundle is embedded. Neither phone check can be verified on this machine.

## The Android floor

**minSdkVersion 28 — Android 9+.** PRODUCT.md fixes it; `expo-build-properties`
enforces it when `expo prebuild` generates the native project.
