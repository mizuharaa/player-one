 ...
 *
 * **There is no SVG here.** The drawing predates the first native build and
 * `react-native-svg` never became a dependency, so every shape below is a
 * `View`: a circle is a View whose `borderRadius` is half its
...
export function Panda({
  ...
  // Loops stop for "remove animations" (and while that answer is still
  // unknown) and whenever the app is not in front: a breathing panda behind
  // another app is battery spent on nothing. Both hooks run unconditionally —
  // a `||` between them would skip the second on the first render and change
  // the hook order once the setting resolves, which React refuses.
  const reducedMotion = useReducedMotion();
  const active = useAppActive();
  const reduced = reducedMotion !== false || !active;
