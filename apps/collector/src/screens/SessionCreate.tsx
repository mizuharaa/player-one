...
export function SessionCreate() {
  ...
  return <Screen progress={progress} title={tt(titles[step]!)} onBack={() => step > 0 ? setStep(step - 1) : nav.back()}
    ...
    {create.isError ? <Failure onRetry={next} busy={create.isPending} error={create.error} tone="error" text={tt(SESSION_ERRORS[create.error.message] ?? 'common.actionFailed')} /> : null}
