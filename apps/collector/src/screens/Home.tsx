...
export function Home() {
  ...
  return <Screen refresh={{ refreshing: [profile, devices, episodes, tasks, sessions, income, cycle].some(q => q.isRefetching), onRefresh: () => { for (const q of [profile, devices, episodes, tasks, sessions, income, cycle]) void q.refetch(); } }} title={tt(`greeting.${mascotStateAt()}`)} right={
        ...
        showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: c.cardGap }} renderItem={({ item }) =>
          <View style={{ width: (width - 2 * c.gutter) * 0.86 }}><Card>
            {item === 'device' ? <>
              ...
              {sessions.isError ? <Failure error={sessions.error} tone="error" text={tt('common.loadFailed')} onRetry={() => void sessions.refetch()} busy={sessions.isFetching} /> : sessions.isPending ? <Loading /> : !todaySession ? <Body muted>{tt('home.noSessionToday')}</Body> : <Body muted>{tt(`scenario.${todaySession.scenario}`)}</Body>}
            </> : <>
              <NavRow label={tt('home.awaiting')} subtitle={awaiting ? shortId(awaiting.episodeId) : undefined} onPress={() => nav.selectTab('income')} />
              {income.isError ? <Failure error={income.error} tone="error" text={tt('common.loadFailed')} onRetry={() => void income.refetch()} busy={income.isFetching} /> : income.isPending ? <Loading /> : awaiting ? <Text style={{ ...c.type.h2, fontFamily: face(theme), color: c.greenInk }}>{awaiting.amountVnd === null ? '—' : dong(awaiting.amountVnd)}</Text> : <Body muted>{tt('home.noAwaiting')}</Body>}
            </>}
          </Card></View>} />
    </View>
    ...
    <View style={{ gap: c.cardGap, paddingTop: c.sectionGap }}>
      ...
      {episodes.isError ? <Failure error={episodes.error} tone="error" text={tt('common.loadFailed')} onRetry={() => void episodes.refetch()} busy={episodes.isFetching} /> : null}
