...
export function Home() {
  ...
  return <Screen refresh={{ refreshing: [profile, devices, episodes, tasks, sessions, income, cycle].some(q => q.isRefetching), onRefresh: () => { for (const q of [profile, devices, episodes, tasks, sessions, income, cycle]) void q.refetch(); } }} title={tt(`greeting.${mascotStateAt()}`)} right={
    ...
    {failed ? <Failure error={failed.error} text={tt('common.loadFailed')} onRetry={retry} busy={queries.some(q => q.isFetching)} /> : null}
    <HeaderGradient>
    <BrandSlot />
    <Text style={{ ...c.type.h1, fontFamily: face(theme), color: c.ink }}>{profile.data?.name ?? ''}</Text>
    {profile.isError ? <Body muted>{tt('common.loadFailed')}</Body> : null}
    {cycle.isError ? <Body muted>{tt('common.loadFailed')}</Body> : <View ref={earningsTarget} collapsable={false} style={{ gap: theme.space[2], paddingVertical: c.cardPad }}>
        ...
        showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: c.cardGap }} renderItem={({ item }) =>
          <View style={{ width: (width - 2 * c.gutter) * 0.76 }}><Card>
            {item === 'device' ? <>
              ...
              {devices.isError ? <Body muted>{tt('common.loadFailed')}</Body> : devices.isPending ? <Loading /> : devices.data?.length === 0 ? <Body muted>{tt('session.needDevice')}</Body> : null}
            </> : item === 'today' ? <>
              <NavRow label={tt('home.today')} subtitle={todaySession ? shortId(todaySession.id) : undefined} onPress={() => nav.selectTab('uploads')} />
              {sessions.isError ? <Body muted>{tt('common.loadFailed')}</Body> : sessions.isPending ? <Loading /> : !todaySession ? <Body muted>{tt('home.noSessionToday')}</Body> : <Body muted>{tt(`scenario.${todaySession.scenario}`)}</Body>}
            </> : <>
              <NavRow label={tt('home.awaiting')} subtitle={awaiting ? shortId(awaiting.episodeId) : undefined} onPress={() => nav.selectTab('income')} />
              {income.isError ? <Body muted>{tt('common.loadFailed')}</Body> : income.isPending ? <Loading /> : awaiting ? <Text style={{ ...c.type.h2, fontFamily: face(theme), color: c.greenInk }}>{awaiting.amountVnd === null ? '—' : dong(awaiting.amountVnd)}</Text> : <Body muted>{tt('home.noAwaiting')}</Body>}
            </>}
          </Card></View>} />
    </View>
    <View ref={tasksTarget} collapsable={false} style={{ gap: c.cardGap }}>
      <Title>{tt('home.recommended')}</Title>
      {tasks.isError ? <Body muted>{tt('common.loadFailed')}</Body> : null}
