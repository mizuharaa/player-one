...
export function PhantomPressable({ style, onPressIn, onPressOut, onPress, ...props }: PressableProps) {
  ...
  return <Target {...props} style={[typeof style === 'function' ? style({ pressed }) : style, motion]}
    onPressIn={event => { setPressed(true); scale.value = withTiming(.97, { duration: 120 }); onPressIn?.(event); }}
