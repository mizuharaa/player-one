...
vi.mock('react-native-reanimated', async () => ({ ...await vi.importActual('../test/reanimated.tsx'), default: { View: ({ entering, exiting, children }: { entering: unknown; exiting: unknown; children: React.ReactNode }) => { seen.push({ entering, exiting, initialX: (entering as (v: { windowWidth: number }) => { initialValues: { transform: { translateX?: number }[] } })({ windowWidth: 390 }).initialValues.transform[0]?.translateX }); return <div>{children}</div>; } } }));
