...
test('the TestFlight profile is a store-distributed demo at version 45', () => {
  ...
  for (const name of ['demo', 'store', 'testflight']) {
    assert.equal(profileEnv(name).PLAYERONE_VERSION_CODE, '45', `${name} must be at version 45`);
  }
  ...
  assert.equal(env.PLAYERONE_VERSION_CODE, '45');
  ...
  assert.equal(value.ios.buildNumber, '45');
  assert.equal(value.android.versionCode, 45);
