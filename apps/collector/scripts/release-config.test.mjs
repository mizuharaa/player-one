...
test('the TestFlight profile is a store-distributed demo at version 48', () => {
  ...
  for (const name of ['demo', 'store', 'testflight']) {
    assert.equal(profileEnv(name).PLAYERONE_VERSION_CODE, '48', `${name} must be at version 48`);
  }
  ...
  assert.equal(env.PLAYERONE_VERSION_CODE, '48');
  ...
  assert.equal(value.ios.buildNumber, '48');
  assert.equal(value.android.versionCode, 48);
