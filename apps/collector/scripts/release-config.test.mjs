...
test('the TestFlight profile is a store-distributed demo at version 51', () => {
  ...
  for (const name of ['demo', 'store', 'testflight']) {
    assert.equal(profileEnv(name).PLAYERONE_VERSION_CODE, '51', `${name} must be at version 51`);
  }
  ...
  assert.equal(env.PLAYERONE_VERSION_CODE, '51');
  ...
  assert.equal(value.ios.buildNumber, '51');
  assert.equal(value.android.versionCode, 51);
