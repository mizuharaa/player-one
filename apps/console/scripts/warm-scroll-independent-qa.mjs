...
const BASE=process.env.QA_BASE||'http://127.0.0.1:5190',OUT='scratchpad/qa/warm-scroll';mkdirSync(OUT,{recursive:true});
...
await withBrowser(async browser=>{
 for(const width of [375,1440,1920]){
  ...
  await p.addInitScript(()=>localStorage.setItem('playerone:showcase-cookie-choice:v1','declined'));await p.goto(BASE+'/discover');await p.waitForSelector('.discover-page[data-logo-complete="true"]');await p.waitForSelector('.logo-intro',{state:'detached'});await p.waitForTimeout(1000);
  const run={width,opening:await inspect(p),stops:[]};
  for(const target of ['.discover-aperture-section','.discover-work']){
 ...
 const {page:p,context}=await newPage(browser,{viewport:{width:375,height:900}});await p.addInitScript(()=>localStorage.setItem('playerone:showcase-cookie-choice:v1','declined'));await p.goto(BASE+'/discover');await p.waitForSelector('.discover-page');await p.mouse.wheel(0,1600);await p.waitForTimeout(500);out.reduced=await inspect(p);out.reduced.intro=await p.locator('.logo-intro').count();await context.close();
