...
/** "Step 2 of 5" — the cheapest read of which step is showing. */
function stepText(): string {
  return container.querySelector('dialog p')?.textContent ?? '';
}

/** Close tour, Back, Next — in DOM order. */
function buttons(): HTMLButtonElement[] {
  return Array.from(container.querySelectorAll<HTMLButtonElement>('dialog button'));
}
...
/**
 * Enter is the tour's own "Next", which means Enter is also the key every
 * focused button in the tour answers to. The first version preventDefault-ed
 * and advanced on every Enter, so Enter with "Back" focused went forward: the
 * control said one thing and did another, and on the last step "Back" would
 * have ended the tour.
 */
describe('Enter belongs to the focused button', () => {
  for (const [label, index] of [
    ['Close tour', 0],
    ['Back', 1],
    ['Next', 2],
  ] as const) {
    it(`Enter on "${label}" does not advance the tour behind its back`, () => {
      act(() => {
        startGuide('/');
      });
      /* Move off step 1 so both directions are observable. */
      press(dialogNode(), 'ArrowRight');
      ...
      /* The tour did not step, and nothing reached the review screen. */
      expect(stepText()).toContain('2');
      expect(seen).toEqual([]);
    });
  }

  it('Enter still advances when the focus is not on a button', () => {
    act(() => {
      startGuide('/');
    });
    press(dialogNode(), 'Enter');
    expect(stepText()).toContain('2');
    expect(seen).toEqual([]);
  });
});
 ...
 *
 * The offer strip on Home unmounts itself in the same click that starts the
 * tour, so an opener captured in an effect is `<body>` and the focus is lost.
 * It is captured synchronously in `startGuide` instead, and when that element
 * has gone the shell's permanent trigger takes the focus.
 */
describe('focus returns to whoever asked for the tour', () => {
  it('to the element that started it', () => {
    ...
    /* The dialog took the focus — otherwise the next line proves nothing. */
  ...
  it('to the shell trigger when the opener has gone', () => {
    const shellTrigger = document.createElement('button');
    shellTrigger.setAttribute('data-guide', 'shell.guide');
    document.body.append(shellTrigger);
    ...
    act(() => {
      startGuide('/');
      /* The offer strip dismisses itself in the same click. */
      offer.remove();
    });
    expect(document.activeElement).not.toBe(shellTrigger);
    press(dialogNode(), 'Escape');
    expect(document.activeElement).toBe(shellTrigger);
    shellTrigger.remove();
  });
});
